...
export class ChefsService {
  ...
  updateChefHandler(responseChef: IChef) {
    ...
    const updatedChefIndex = this.chefs.findIndex(
      (chef) => chef._id === responseChef._id
    );

    if (responseChef.chefOfTheWeek) this.removeChefOfTheWeek();
  ...
  postChef(chef: IChef) {
    this.http
      .post(this.API, chef, this.authService.authorizationHeader)
      .subscribe({
  ...
  deleteChef(chef: IChef) {
    this.http
      .delete(`${this.API}/${chef._id}`, this.authService.authorizationHeader)
      .subscribe({
  ...
  deleteChefHandler(response: { message: string; id: string }) {
    ...
    const deletedChefIndex = this.chefs.findIndex(
      (chef) => chef._id === response.id
    );
